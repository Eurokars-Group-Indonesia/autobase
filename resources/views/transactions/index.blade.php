...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Handle view details inline button click (when filtering - toggle inline)
        $(document).on('click', '.view-details-inline', function(e) {
            e.preventDefault();
            const $button = $(this);
            const wipNo = $button.data('wipno');
            const invNo = $button.data('invno');
            const brandCode = $button.data('brandcode');
            const magicId = $button.data('magicid');
            const headerId = $button.data('headerid');
            const $detailsRow = $('#details-' + headerId);
            const $icon = $button.find('i');
            
            console.log('View details inline clicked:', wipNo, invNo, magicId);
            ...
            if ($detailsRow.is(':visible')) {
                ...
                // Show details and load data if not already loaded
                $detailsRow.show();
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success && response.data.length > 0) {
                                ...
                                html += '<th class="text-center">Part No</th>';
                                html += '<th class="text-center">Description</th>';
                                html += '<th class="text-center">Date Decard</th>';
                                html += '<th class="text-center">Qty</th>';
                                html += '<th class="text-center">Selling Price</th>';
                                html += '<th class="text-center">Discount %</th>';
                                html += '<th class="text-center">Extended Price</th>';
                                html += '<th class="text-center">Part/Labour</th>';
                                html += '</tr></thead><tbody>';
                                ...
                                response.data.forEach(function(item, index) {
                                    ...
                                    html += '<td class="text-center">' + dateDecard + '</td>';
                                    html += '<td class="text-end">' + parseFloat(item.qty || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</td>';
                                    html += '<td class="text-end">' + parseFloat(item.selling_price || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</td>';
                                    html += '<td class="text-end">' + parseFloat(item.discount || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '%</td>';
                                    html += '<td class="text-end">' + parseFloat(item.extended_price || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</td>';
                                    html += '<td class="text-center"><span class="badge bg-' + (item.part_or_labour === 'P' ? 'primary' : 'success') + '">';
                                    html += (item.part_or_labour === 'P' ? 'Part' : 'Labour') + '</span></td>';
                                    html += '</tr>';
                                });
                                
                                html += '</tbody><tfoot class="table-secondary"><tr>';
                                html += '<th colspan="7" class="text-end">Total:</th>';
                                html += '<th class="text-end">' + totalExtPrice.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</th>';
                                html += '<th></th></tr></tfoot></table></div>';
                                
                                $detailsRow.find('.body-details-content').html(html);
                                $detailsRow.data('loaded', true);
                            } else {
                                $detailsRow.find('.body-details-content').html('<div class="alert alert-info mb-0">No transaction body details found.</div>');
                                $detailsRow.data('loaded', true);
                            }
                        },
                        error: function(xhr) {
                            $detailsRow.find('.body-details-content').html('<div class="alert alert-danger mb-0">Failed to load transaction details. Please try again.</div>');
